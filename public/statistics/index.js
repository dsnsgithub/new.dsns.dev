...
async function refreshPlayerList() {
	...
	for (const uuid in JSON.parse(window.localStorage.getItem("UUIDs"))) {
		...
		button.innerHTML += `<button style="position: absolute; left: 75%;" class="button is-ghost" onClick="remove('${uuid}')">❌</button>`;
...
async function loadChart() {
	...
	for (const uuid in storage) {
		...
		if (historyCache[uuid]) {
			...
			response = await fetch(`https://cors.dsns.dev/old.dsns.dev/api/history/${uuid}`).then((res) => res.json());
